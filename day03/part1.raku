...
=item The first rucksack contains the items vJrwpWtwJgWrhcsFMMfFFhFp, which
...
=item The second rucksack's compartments contain jqHRNqRjqzjGDLGL and
  rsFMfFZSrLrFZsSL. The only item type that appears in both compartments is
  uppercase L.
=item The third rucksack's compartments contain PmmdzqPrV and vPwwTWBwg; the
  only common item type is uppercase P.
=item The fourth rucksack's compartments only share item type v.
=item The fifth rucksack's compartments only share item type t.
=item The sixth rucksack's compartments only share item type s.
...
sub priority($a) { $a <= 'Z'.ord ?? $a - 'A'.ord + 27 !! $a - 'a'.ord  + 1 }

my $sum = 0;
for 'input.txt'.IO.lines -> $line {
  my @items = $line.ords;
  my $n = @items.elems div 2;
  my $sack1 = @items[0..^$n].Set;
  my $dup = @items[$n..*].first: * (elem) $sack1;
  $sum += priority($dup);
}

say "Sum of priorities: $sum";
