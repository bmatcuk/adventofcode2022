...
=item bvwbjplbgvbhsrlpgdmjqwftvncz: first marker after character 5
=item nppdvjthqldpwncqszvftbrmjlhg: first marker after character 6
=item nznrnfrfntjfmvfwmzdfjlvtqnbhcprsg: first marker after character 10
=item zcfzfwzzqfrljwzlrfnpqdbhtmscgvjw: first marker after character 11
...
for 'input.txt'.IO.comb -> $char {
  @chars[$marker mod 4] = $char;
  $marker++;
  next if $marker < 4;
  last if @chars[1] ne @chars[0]
    && @chars[2] ne @chars[1] && @chars[2] ne @chars[0]
    && @chars[3] ne @chars[2] && @chars[3] ne @chars[1] && @chars[3] ne @chars[0];
}
