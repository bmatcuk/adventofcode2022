...
# load input - give us a few extra lines in the y direction just in case sand
# piles up to the top
my @lines = 'input.txt'.IO.lines.map({ Line.parse($_, actions => LineBuilder).made }).list;
my $xrange = @lines.map({ $_.map(*.x) }).flat.minmax;
my $yrange = @lines.map({ $_.map(*.y) }).flat.minmax;
my $xadj = $xrange.min;
my $yadj = $yrange.min - 10;
$xrange -= $xadj;
$yrange -= $yadj;
...
for @lines -> @line {
  ...
  for @line.skip -> $point {
    for ($point.y, $prev-point.y).minmax - $yadj -> $y {
