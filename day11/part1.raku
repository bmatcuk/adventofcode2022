...
=item Starting items lists your worry level for each item the monkey is
  currently holding in the order they will be inspected.
=item Operation shows how your worry level changes as that monkey inspects an
  ...
  inspection.)
=item Test shows how the monkey uses your worry level to decide where to throw
  an item next.
=item2  If true shows what happens with an item if the Test was true.
=item2  If false shows what happens with an item if the Test was false.
...
class Monkey {
  ...
  method throw-items (@monkeys) {
    for @!items -> $item {
      my $worry = &!operation($item) div 3;
...
class Monkeys {
  ...
  method run ($rounds) {
    for ^$rounds {
      .throw-items(@!monkeys) for @!monkeys;
    }
  }
}
