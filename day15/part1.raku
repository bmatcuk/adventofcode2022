...
my SetHash $covered .= new;
my SetHash $beacons-in-row .= new;
...
for @readings -> $reading {
  ...
  # (|) works but has terrible performance ¯\_(ツ)_/¯
  my $diff = ($distance-to-beacon - $distance-to-row).abs;
  my $range = ($reading.point.x - $diff)..($reading.point.x + $diff);
  say $range.raku;
  $covered (|)= $range;
  $beacons-in-row.set($reading.beacon.x) if $reading.beacon.y == $row;
}

say "{$covered.elems - $beacons-in-row.elems} positions cannot contain a beacon in row $row";
