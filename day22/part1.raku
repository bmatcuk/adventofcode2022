...
=item A number indicates the number of tiles to move in the direction you are
  facing. If you run into a wall, you stop moving forward and continue with the
  next instruction.
=item A letter indicates whether to turn 90 degrees clockwise (R) or
...
sub next-position {
  ...
  given $heading {
    when 0 {
      $new-x++;
      $new-x = @input[$new-y].first({ $_ ne ' ' }, :k) if wrap($new-x, $new-y);
    }
    when 1 {
      $new-y++;
      $new-y = @input.first({ $x < $_.elems && $_[$x] ne ' ' }, :k) if wrap($new-x, $new-y);
    }
    when 2 {
      $new-x--;
      $new-x = @input[$new-y].first({ $_ ne ' ' }, :k, :end) if wrap($new-x, $new-y);
    }
    when 3 {
      $new-y--;
      $new-y = @input.first({ $x < $_.elems && $_[$x] ne ' ' }, :k, :end) if wrap($new-x, $new-y);
    }
  }
  return ($new-x, $new-y);
}
