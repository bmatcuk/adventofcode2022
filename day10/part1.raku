...
=item addx V takes two cycles to complete. After two cycles, the X register is
  increased by the value V. (V can be negative.)
=item noop takes one cycle to complete. It has no other effect.
...
=item At the start of the first cycle, the noop instruction begins execution.
  During the first cycle, X is 1. After the first cycle, the noop instruction
  finishes execution, doing nothing.
=item At the start of the second cycle, the addx 3 instruction begins
  execution. During the second cycle, X is still 1.
=item During the third cycle, X is still 1. After the third cycle, the addx 3
  instruction finishes execution, setting X to 4.
=item At the start of the fourth cycle, the addx -5 instruction begins
  execution. During the fourth cycle, X is still 4.
=item During the fifth cycle, X is still 4. After the fifth cycle, the addx -5
...
=item During the 20th cycle, register X has the value 21, so the signal
  ...
  = 21.)
=item During the 60th cycle, register X has the value 19, so the signal
  strength is 60 * 19 = 1140.
=item During the 100th cycle, register X has the value 18, so the signal
  strength is 100 * 18 = 1800.
=item During the 140th cycle, register X has the value 21, so the signal
  strength is 140 * 21 = 2940.
=item During the 180th cycle, register X has the value 16, so the signal
  strength is 180 * 16 = 2880.
=item During the 220th cycle, register X has the value 18, so the signal
...
class Runner {
  ...
  method noop ($/) { $!cycle++; self!check-signal }
  method !check-signal {
    if $!cycle == 20 || ($!cycle - 20) %% 40 {
      my $strength = $!cycle * $!x;
      say "$!cycle * $!x: $strength";
      $!signal-strength += $strength;
    }
  }
}

my $actions = Runner.new;
for 'input.txt'.IO.lines -> $line {
  OpCodes.parse($line, actions => $actions);
}
say "Sum of signal strengths: {$actions.signal-strength}";
