...
=item1 cd means change directory. This changes which directory is the current
  directory, but the specific result depends on the argument:
=item2   cd x moves in one level: it looks in the current directory for the
           directory named x and makes it the current directory.
=item2   cd .. moves out one level: it finds the directory that contains the
           current directory, then makes that directory the current directory.
=item2   cd / switches the current directory to the outermost directory, /.
=item1 ls means list. It prints out all of the files and directories
  immediately contained by the current directory:
=item2   123 abc means that the current directory contains a file named abc
           with size 123.
=item2   dir xyz means that the current directory contains a directory named
           xyz.
...
=item The total size of directory e is 584 because it contains a single file i
  of size 584 and no other directories.
=item The directory a has total size 94853 because it contains files f (size
  29116), g (size 2557), and h.lst (size 62596), plus file i indirectly (a
  contains e which contains i).
=item Directory d has total size 24933642.
=item As the outermost directory, / contains every file. Its total size is
...
say "Total size of directories less than 100,000: {$actions.sizes.values.grep(* <= 100000).sum}";
