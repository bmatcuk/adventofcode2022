...
constant $digits = Map.new('2', 2, '1', 1, '0', 0, '-', -1, '=', -2);
constant $decimals = Map.new(0, '0', 1, '1', 2, '2', 3, '=', 4, '-');

my $sum = 'input.txt'.IO.lines.map({
  $^line.comb.kv.map({ 5 ** ($line.chars - $^i - 1) * $digits{$^v} }).sum
}).sum;

my @snafu of Str;
while $sum > 0 {
  my $digit = $sum % 5;
  @snafu.push: $decimals{$digit};
  $sum div= 5;
  $sum++ if $digit > 2;
}
say "SNAFU: {@snafu.reverse.join('')}";
