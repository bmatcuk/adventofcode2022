...
class Moves {
  has $!head = Point.new(x => 0, y => 0);
  has $!tail = Point.new(x => 0, y => 0);
  has SetHash $.visited .= new($!head);
  ...
  method !move-head ($x, $y) {
    my ($headx, $heady) = $!head.kv;
    for ^$x.abs {
      $headx += $x.sign;
      self!move-tail: $headx, $heady;
    }
    for ^$y.abs {
      $heady += $y.sign;
      self!move-tail: $headx, $heady;
    }
    $!head = Point.new(x => $headx, y => $heady);
  }
  method !move-tail ($headx, $heady) {
    my ($tailx, $taily) = $!tail.kv;
    my $diffx = $headx - $tailx;
    my $diffy = $heady - $taily;
    if $diffx.abs > 1 || $diffy.abs > 1 {
      # should never be a case where they are both >1
      if $diffx.abs > 1 {
        $tailx += $diffx.sign;
      ...
      $!tail = Point.new(x => $tailx, y => $taily);
      $.visited.set: $!tail;
    }
  }
}
